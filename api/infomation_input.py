...
class InformationInput():
	def __init__(self, path : str):
		self.convertdirpath = os.getcwd() + '/after_conversion'
		self.tmpdirpath = os.getcwd() + '/tmp'
		if not os.path.exists(self.convertdirpath):
			os.mkdir(self.convertdirpath)
		if not os.path.exists(self.tmpdirpath):
			os.mkdir(self.tmpdirpath)
		self.lsts = []
		self.expath = path
		self.cpfile = 'cpexfile.xlsx'
		self.name = 0
		self.column = 1
		self.row = 2

	def iter_cols(self):
		cppath = shutil.copy(self.expath, self.tmpdirpath)
	...
	def create_excel(self):
		filepath = self.convertdirpath + '/' + Path(self.expath).stem + '.xlsx'
		self.cxpath = shutil.copy('./format.xlsx', filepath)
		wb = openpyxl.load_workbook(self.cxpath)
		sheet = wb[wb.sheetnames[0]]
		self.get_format_info(sheet)
		print(self.lsts)
		for lst in self.lsts:
			size = len(lst[3])
			if lst[self.name] == '単価':
	...
	def get_format_info(self, sheet):
		self.flsts = []
		for	cols in sheet.iter_cols():
			for	cell in cols:
				if cell.value == '単価':
		...
		self.div_format_info(self.flsts)
		print(self.flsts)

	def div_format_info(self, flsts : list):
		for flst in flsts:
